...
function publicMessage(err: unknown, fallback: string) {
...
async function loadBillingProfile(userId: string) {
  const billed = await admin()
    ...
    .eq('id', userId)
    .maybeSingle()
  if (!billed.error) return billed.data
  console.error('Profile billing select failed:', billed.error.message)
...
export async function startCheckout(input: { accessToken: string; promoCode?: string }) {
  try {
    ...
    return { error: publicMessage(err, 'Could not start checkout.') }
  }
}
...
export async function startPortal(input: { accessToken: string }) {
  try {
    ...
    return { error: publicMessage(err, 'Could not open billing.') }
  }
}
