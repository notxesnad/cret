      ...
      <div className="flex-none h-[72px] flex items-center px-6 border-b border-slate-800 bg-slate-900 z-10 pt-safe">
        ...
        {hub === 'work' && (
          <div className="flex-1 mx-4 bg-slate-800 rounded-full h-3 overflow-hidden">
            <div
              className="bg-seller h-full rounded-full transition-all duration-500 ease-out"
      ...
      {hub === 'menu' ? (
        <div className="flex-1 min-h-0 overflow-y-auto hide-scrollbar bg-slate-900">
          <ToolLanding
            ...
            primaryEmoji="📋"
            primaryClass="group relative bg-seller hover:bg-seller-hover text-slate-950 p-6 rounded-3xl shadow-xl flex flex-col justify-between min-h-[120px] overflow-hidden"
            onPrimary={() => setHub('work')}
            onHow={() => {
              setHowPage(0)
              setHub('how')
            }}
            howClass="group relative bg-seller-soft hover:bg-white text-slate-900 p-6 rounded-3xl shadow-xl flex flex-col justify-between min-h-[120px] overflow-hidden border-2 border-transparent hover:border-seller"
          />
        </div>
      ) : hub === 'how' ? (
        ...
        <div className="absolute inset-0 flex transition-transform duration-500 ease-in-out h-full" style={{ width: '300%', transform: step === 1 ? 'translateX(0%)' : step === 2 ? 'translateX(-33.333333%)' : 'translateX(-66.666667%)' }}>
          ...
          <div className="w-[33.333333%] flex-shrink-0 px-6 py-6 h-full overflow-y-auto hide-scrollbar">
            <div className="text-center mb-8">
              <span className="text-xs font-bold tracking-widest text-seller uppercase font-seller">Tracker Report</span>
            ...
            <div className="space-y-3">
              ...
              {listings.filter(listing => !isSellerDemoListing(listing)).map(listing => (
                <div 
                  ...
                  className="bg-slate-800 border border-slate-700 rounded-xl p-4 flex justify-between items-center group cursor-pointer hover:border-seller/50 transition"
                >
                  <div>
                  ...
                  <div className="text-slate-500 group-hover:text-seller transition">
              ...
              {listings.filter(isSellerDemoListing).map(listing => (
                ...
                >
                  <div>
                    <span className="text-[10px] font-black uppercase tracking-wider text-seller bg-seller/10 px-2 py-0.5 rounded">Demo</span>
              ...
              <>
                ...
                <div className="mb-6">
                  <button
                    ...
                    className="w-full bg-slate-800 hover:bg-slate-700 text-seller border border-slate-700 font-black py-4 rounded-xl transition shadow flex items-center justify-center gap-2"
                  >
                      ...
                      <div className="bg-slate-800/50 p-4 rounded-2xl border border-slate-700 mt-4">
                        ...
                        <div className="flex gap-2 mb-4">
                          <input
                            ...
                            className="flex-1 bg-slate-800 border border-slate-600 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-seller transition-colors"
                          />
                          <button
                            onClick={() => handleAddActivity(customActivity)}
                            className="bg-seller hover:bg-seller-hover active:scale-95 text-slate-950 font-bold px-4 py-2 rounded-xl transition-all duration-150"
                          >
                            Add
                          </button>
                        </div>
                        ...
                        <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2">
                          {PRESET_ACTIVITIES.map(preset => (
                            <button
                              ...
                              className="w-full text-left bg-slate-900 hover:bg-slate-700 active:bg-seller active:text-slate-950 border border-slate-700 p-3 rounded-lg transition"
                            >
                ...
                <div className="space-y-3 scroll-mt-6" ref={activityLogRef}>
                  <h3 className="text-sm font-bold text-white flex items-center justify-between" ref={activityLogHeaderRef}>
                    Activity Log
                    <span className="bg-seller/20 text-seller px-2 py-0.5 rounded text-[10px] uppercase tracking-wider">{activeListing.activities.length} total</span>
                  </h3>
                    ...
                    <div className="space-y-2">
                      {sortActivitiesNewestFirst(activeListing.activities).map((act) => (
                        <div 
                          ...
                          className={`bg-slate-800 border ${act.status === 'pending' ? 'border-amber-500/50 border-dashed' : act.status === 'upcoming' ? 'border-cyan-500/50' : 'border-slate-700'} rounded-xl p-3 flex justify-between items-start group cursor-pointer hover:border-seller transition-colors`}
                        >
                          <div>
                          ...
                          <span
                            className="text-slate-400 group-hover:text-seller p-1 ml-3 transition"
                            title="Edit activity"
                          >
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-sm font-bold text-white focus:outline-none focus:border-seller transition-colors" 
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea 
                      ...
                      className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-seller transition-colors min-h-[100px]" 
                    />
                  </div>
                </div>
              </>
        ...
        <div className="flex-none p-6 bg-slate-900 border-t border-slate-800 z-10 pb-safe">
          <div className="flex gap-3">
            ...
            <button 
              ...
              className="flex-[2] bg-seller hover:bg-seller-hover text-slate-950 font-black py-4 rounded-xl transition shadow"
            >
              Save Changes
            </button>
          </div>
