...
export function HomeView({ switchView, showCustomModal }: { switchView: (view: string) => void, showCustomModal: (msg: string) => void }) {
  ...
  return (
    <div id="view-home" className="app-view active space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-20 mt-[-60px]">
      ...
      <div className="mt-12 bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl text-center relative overflow-hidden">
        ...
        <a href="#view-home" onClick={() => switchView('home')} className="block w-full bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black py-4 px-6 rounded-xl transition shadow-lg text-center uppercase tracking-wide text-sm relative z-10 mb-4">
          Start your 14-day free trial
        </a>

        {/* Support & Ideas Button */}
        <a 
          href="mailto:hello@coolrealestatetools.com?subject=Tool%20Idea%20or%20Help" 
          className="block w-full bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-slate-600 text-white font-bold py-4 px-6 rounded-xl transition text-center text-sm relative z-10 flex items-center justify-center gap-2"
        >
